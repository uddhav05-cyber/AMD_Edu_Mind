...
export default function App() {
  ...
  const DashboardScreen = () => (
    <div className={`min-h-screen bg-[#07090F] text-white pb-24 animate-fade-in transition-opacity duration-300 ${navLoading ? 'opacity-50' : 'opacity-100'}`}>
      ...
      <div className="px-6 mt-6">
        ...
        {/* placeholder buttons for new features */}
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button onClick={() => navigate('quiz')} className="p-2 bg-[#0A84FF] rounded">Generate quiz</button>
          <button onClick={() => navigate('planner')} className="p-2 bg-[#FF9F0A] rounded">Study planner</button>
        </div>
      </div>
      <div className="px-6 space-y-6 mt-6">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          <button onClick={() => navigate('planner')} className="p-2 bg-[#0A84FF] rounded text-[10px] font-mono">Study planner</button>
          <button onClick={() => navigate('quiz')} className="p-2 bg-[#5E5CE6] rounded text-[10px] font-mono">Generate quiz</button>
        </div>
      </div>
  ...
  const PlannerScreen = () => (
    <div className="min-h-screen bg-[#07090F] text-white p-6 flex flex-col items-center justify-center">
      <h2 className="text-2xl font-serif font-bold mb-4">Study Planner (stub)</h2>
      <p className="text-center">This will show a schedule generated by the backend.</p>
      <button onClick={() => navigate('dashboard')} className="mt-6 py-3 px-8 bg-[#E8232A] rounded-xl">Back</button>
    </div>
  );

  const QuizScreen = () => {
    // In the future this will call /api/quiz; for now reuse static data
